  ...
  if (tasks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 bg-white rounded-xl shadow-sm border border-dashed border-gray-300">
        <Inbox className="h-16 w-16 text-gray-300 mb-4 animate-bounce" />
        <h3 className="text-lg font-semibold text-gray-700">Không tìm thấy kết quả nào</h3>
  ...
  return (
    <div className="space-y-3">
      {tasks.map((task) => (
        <div 
          key={task.id} 
          className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 bg-white rounded-xl shadow-sm border border-gray-100 hover:border-indigo-100 hover:shadow-md transition duration-200 gap-4"
        >
          {/* Thông tin Task */}
          <div className="space-y-1.5">
            <Link 
              to={`/task/${task.id}`} 
              className="font-bold text-gray-800 text-base hover:text-indigo-600 hover:underline transition"
            >
              {task.title}
            </Link>
          ...
          <div className="flex items-center gap-1 self-end sm:self-center">
            <Link 
              ...
              className="flex items-center gap-1 px-3 py-1.5 text-xs font-bold text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition"
